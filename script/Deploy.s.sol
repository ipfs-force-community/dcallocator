...
contract DeployScript is Script {
    ...
    // 从环境变量中读取配置
    function getConfig() internal view returns (uint256 threshold, address vault, uint256 challengePeriod) {
        // 从环境变量获取阈值
    ...
    function run() public {
        (uint256 threshold, address vault, uint256 challengePeriod) = getConfig();
    ...
    function splitString(string memory str, string memory delimiter) internal pure returns (string[] memory) {
        ...
        for (uint256 i = 0; i <= strBytes.length; i++) {
            if (
                i == strBytes.length
                    || (i <= strBytes.length - delimiterBytes.length && isSubstringAt(strBytes, delimiterBytes, i))
            ) {
    ...
    // 辅助函数：检查子字符串是否在指定位置
    function isSubstringAt(bytes memory str, bytes memory substr, uint256 pos) internal pure returns (bool) {
        if (pos + substr.length > str.length) {
            return false;
        }

        for (uint256 i = 0; i < substr.length; i++) {
